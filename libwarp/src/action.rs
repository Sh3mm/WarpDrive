...
pub fn gen_action_list(local: &Vec<RFileInfo>, remote: &Vec<RFileInfo>, ledger: &Ledger) -> Vec<Action> {
    ...
    return file_map.iter().map(
        |(p, times)| to_action(p ,&times[0], &times[1], &ledger)
    ).collect::<Vec<Action>>()
}
...
fn to_action(path: &String, local: &Option<OffsetDateTime>, remote: &Option<OffsetDateTime>, ledger: &Ledger) -> Action {
    ...
    match ledger.path_set.get(path) {
        ...
        None => {
            ...
            if local.is_some() && remote.is_some() {
                return Action::new(path, ActionType::Error(ErrorType::TwoNew))
            }
            // new file in remote
            if local.is_none() && remote.is_some() {
                return Action::new(path, ActionType::Remote2Local)
            }
            // new file in local
            if local.is_some() && remote.is_none() {
                return Action::new(path, ActionType::Local2Remote)
            }
        ...
        Some(_) => {
            ...
            if local.is_none() {
                ...
                if unwrap_remote.cmp(last_update).is_le() {
                    return Action::new(path, ActionType::DelRemote)
                }
                // (no lo) + (re > lu) -> Error
                if unwrap_remote.cmp(last_update).is_gt() {
                    return Action::new(path, ActionType::Error(ErrorType::DelAndMod))
                }
            }
            // remote is None
            if remote.is_none() {
                ...
                if local_time.cmp(last_update).is_le() {
                    return Action::new(path, ActionType::DelLocal)
                }
                // (re > lu) + (no lo) -> Error
                if local_time.cmp(last_update).is_gt() {
                    return Action::new(path, ActionType::Error(ErrorType::ModAndDel))
                }
            }
            ...
            if remote_time.cmp(last_update).is_le() & local_time.cmp(last_update).is_le() {
                return Action::new(path, ActionType::Nothing)
            }
            // (re > lu) + (lo > lu) -> Error
            if remote_time.cmp(last_update).is_gt() & local_time.cmp(last_update).is_gt() {
                return Action::new(path, ActionType::Error(ErrorType::TwoSideMod))
            }
